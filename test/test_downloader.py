import pytest
from unittest.mock import MagicMock, patch, assert_called_once_with,
...
def test_download_obituary_image_success(mock_open, mock_download_image, mock_ensure_directory_exists):
    # Mock configuration
    ...
    with patch("src.downloader.extract_filename_from_url", return_value="19910110_Alt_Russell_Darl"):
        with patch("src.downloader.transform_url_to_image_path", return_value=mock_image_url):
            # Call the function
    ...
    mock_open.assert_called_once_with(Path("/mock/path/1991/19910110_Alt_Russell_Darl.jpg"), "wb")
...
def test_download_obituary_image_invalid_filename(mock_download_image):
    ...
    with patch("src.downloader.extract_filename_from_url", return_value=None):
        with pytest.raises(ValueError, match="Failed to extract filename from URL: http://obit.glbthistory.org/olo/invalid_link"):
            download_obituary_image(obituary_link, mock_client)
...
def test_download_obituary_image_invalid_image_url(mock_download_image):
    ...
    with patch("src.downloader.extract_filename_from_url", return_value="19910110_Alt_Russell_Darl"):
        with patch("src.downloader.transform_url_to_image_path", return_value=None):
            with pytest.raises(ValueError, match="Failed to transform URL to image path: http://obit.glbthistory.org/olo/display.jsp?name=19910110_Alt_Russell_Darl"):
                download_obituary_image(obituary_link, mock_client)
...
def test_download_obituary_image_download_failure(mock_download_image):
    ...
    with patch("src.downloader.extract_filename_from_url", return_value="19910110_Alt_Russell_Darl"):
        with patch("src.downloader.transform_url_to_image_path", return_value=mock_image_url):
            with pytest.raises(ValueError, match="Failed to download image from URL: http://obit.glbthistory.org/olo/imagedb/1991/01/10/19910110_Alt_Russell_Darl/m19910110_0.jpg"):
                download_obituary_image(obituary_link, mock_client)
